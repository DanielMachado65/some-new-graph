...
(() => {
    require(['jquery', 'bootstrap'], ($) => {
        ...
        $('#target').click(() => {
            ...
            $.ajax({
                url: 'https://api.olhonocarro.com.br/auth/login',
                type: 'post',
                data: {
                    username: username,
                    password: pass,
                ...
                .done((resp) => {
                    if (resp && resp.status.cod == 200) {
                        $('body')[0].style.maxWidth = '1280px';
                        $('#loginForm').hide();
                        $('#loader').show();
                        execute();
                    }
...
function execute() {
    require([
        ...
        'locales',
        'handlebarsExtended',
        './integration/api_project.js',
        './integration/api_data.js',
        'prettify',
        'utilsSampleRequest',
        'semver',
        'webfontloader',
        ...
        if (apiProject.footer) {
            ...
            var found_level1 = add_nav(
                nav,
                apiProject.footer.content,
                nav.length,
            ); // Add level 1 and 2 titles
        ...
        function initDynamic() {
            ...
            if (apiProject.template.withCompare) {
                $("#sidenav li[data-version='" + version + "']").each(
                    function () {
        ...
        $('#versions li.version a').on('click', function (e) {
            ...
            $('article[data-version]').each(function (index) {
                ...
                if (semver.lte(version, selectedVersion)) {
                    if (
                        ...
                        $(
                            "#sidenav li.nav-header[data-group='" +
                                group +
                                "']",
                        ).removeClass('hide');
                    }
            ...
            $('article[data-version]').each(function (index) {
                ...
                if (
                    $('section#api-' + group + ' article:visible').length === 0
                ) {
        ...
        function renderArticle(group, name, version) {
            var entry = {};
            $.each(apiByGroupAndName[group][name], function (
                index,
                currentEntry,
            ) {
